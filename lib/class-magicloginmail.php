	...
	public function send_link( $email_arr, $admin_batch ) {
		...
		foreach ( $email_arr as $email ) {
			...
			if ( ! $valid_email ) {
				...
				$message = apply_filters( 'magic_login_mail_message', sprintf( __( 'Hello ! <br><br>Login at %1$s by visiting this url or attached QR Code: <a href="%2$s">%2$s</a>', 'magic-login-mail' ), $blog_name, esc_url( $unique_url ) ), esc_url( $unique_url ), $uid );
